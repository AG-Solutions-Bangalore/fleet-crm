...
const DriverPaymentList = () => {
  ...
  const handleEditDriver = (id) => {
    storeCurrentPage();
    navigate(`/paid-driver/payment-edit/${id}`);
  };
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/driver-payment?${params}`,
        {
          headers: {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const token = Cookies.get("token");
      await axios.delete(`${BASE_URL}/api/driver-payment/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
    ...
    onMutate: async (id) => {
      await queryClient.cancelQueries({ queryKey: ["driverPayments"] });
      const queryKey = [
        "driverPayments",
        debouncedSearchTerm,
    ...
    onSuccess: () => {
      toast.success("Driver Payment deleted successfully");
    },
    onSettled: () => {
  ...
  useEffect(() => {
    ...
    const totalPages = paymentData?.last_page || 1;
  ...
  const columns = [
    ...
    {
      accessorKey: "driver_payment_date",
      id: "Payment Date",
      header: ({ column }) => (
        <Button
          variant="ghost"
      ...
      cell: ({ row }) => (
        <div className="text-[13px]">
          {row.getValue("Payment Date")
            ? moment(row.getValue("Payment Date")).format("DD-MM-YYYY")
            : "—"}
        </div>
      ),
    },
    {
      accessorKey: "driver_payment_full_name",
      id: "Driver Name",
      header: ({ column }) => (
        <Button
          variant="ghost"
  ...
  const table = useReactTable({
    ...
    pageCount: paymentData?.last_page || -1,
        ...
        <div className="flex items-center justify-center h-64 ">
          <div className="text-center ">
            <div className="text-destructive font-medium mb-2">
              Error Fetching Driver Payment List Data
            </div>
    ...
    <div className="max-w-full p-2">
      <div className="flex items-center justify-between py-1">
        <div className="relative w-64">
          ...
          <Input
            placeholder="Search Driver Payments..."
            value={searchTerm}
        ...
        <div className="flex flex-col md:flex-row md:ml-auto gap-2 w-full md:w-auto">
          ...
          <Link to="/paid-driver/create-payment" onClick={storeCurrentPage}>
            <Button variant="default">
              <SquarePlus className="h-3 w-3 mr-2" /> Create Payment
            </Button>
          </Link>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between py-1">
        <div className="text-sm text-muted-foreground">
          Showing {paymentData?.from || 0} to {paymentData?.to || 0} of{" "}
          {paymentData?.total || 0} payments
        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              payment record from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
