          ...
          {data.map((row, i) => (
            ...
            >
              ...
              <td className="border p-2 text-right whitespace-nowrap">
                {Number(row.total_earings || 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
              <td className="border p-2 text-right whitespace-nowrap">
                {Number(row.cash_collected || 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
              <td className="border p-2 text-right whitespace-nowrap">
                {Number(row.deposit_amount || 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
              <td className="border p-2 text-right whitespace-nowrap">
                {Number(row.qr_amount || 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
...
const VehicleWiseSummaryReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/vehicle-wise-summary-report`,
        {
  ...
  return (
    <div className="container mx-auto py-6">
      <Card>
        <CardHeader>
          <CardTitle>Vehicle-wise Summary Report</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Filters Section */}
          ...
          {reportData?.length > 0 && (
            <div className="mt-4">
              <ReportTable data={filteredData} type="Vehicle" />
            </div>
          )}
...
export default VehicleWiseSummaryReport;
