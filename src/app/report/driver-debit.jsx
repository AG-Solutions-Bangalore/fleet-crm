...
const DriverDebitReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/driver-debit-report`,
        formData,
        {
          headers: {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch driver debit report",
      );
      setReportData([]);
    } finally {
      setIsLoading(false);
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("Driver Debits");
      ...
      titleCell.value = "Driver Debit Report";
      ...
      filteredData.forEach((row, index) => {
        ...
        newRow.eachCell((cell, colNumber) => {
          // Alignment and number format for penalty/total columns
          if (colNumber > 5) {
      ...
      saveAs(blob, `driver-debit-report_${fromDate}_to_${toDate}.xlsx`);
                      ...
                      <TableBody>
                        {filteredData.map((row, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell className="text-right px-6 font-bold tabular-nums">
...
export default DriverDebitReport;
