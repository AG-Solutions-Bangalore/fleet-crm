...
const DriverDetailsReport = () => {
  ...
  const fetchReport = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`${BASE_URL}/api/driver-details-report`, {
        headers: {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch driver details report",
      );
      setReportData([]);
    } finally {
      setIsLoading(false);
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      titleCell.value = "Driver Details Report";
      ...
      saveAs(blob, `driver-details-report_${new Date().toISOString().split("T")[0]}.xlsx`);
