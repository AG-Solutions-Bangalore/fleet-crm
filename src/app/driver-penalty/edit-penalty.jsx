...
const EditPenalty = () => {
  ...
  const { data: penaltyData, isLoading: isLoadingPenalty } = useQuery({
    queryKey: ["penalty", id],
    queryFn: async () => {
  ...
  // Sync penalty data to state when loaded (including from cache)
  useEffect(() => {
    if (penaltyData) {
      const d = penaltyData;
  ...
  }, [penaltyData, id, hasInitialized, initialPenalty.id]);
