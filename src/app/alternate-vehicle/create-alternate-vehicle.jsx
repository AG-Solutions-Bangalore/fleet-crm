...
const CreateAlternateVehicle = () => {
  ...
  // Fetch Vehicles (simple list for selection)
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      const response = await axios.post(
        `${BASE_URL}/api/vehicle-alternate-ride`,
        data,
        {
          headers: {
    ...
    onSuccess: (data) => {
      if (data.code === 201 || data.code === 200) {
        ...
        toast.success(data.message || "Alternate Ride Created Successfully");
        navigate("/alternate-vehicle-ride");
      } else {
        toast.error(data.message || "Error creating record");
      }
    },
    onError: (error) => {
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-3 pt-6 border-t border-gray-50">
              <Button
                type="submit"
                disabled={isButtonDisabled || createMutation.isPending}
...
export default CreateAlternateVehicle;
